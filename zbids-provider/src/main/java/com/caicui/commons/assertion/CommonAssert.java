package com.caicui.commons.assertion;

import com.caicui.commons.condition.Condition;
import com.caicui.commons.condition.ConditionBuilder;
...
public class CommonAssert extends Assert {
     ...
     * @return
     */
    public static <T> Boolean isIn(T src, Collection<T> collection) {
        Boolean isIn = false;
        if (CollectionUtils.isEmpty(collection)) return isIn;
        else {
            for (Object file : collection) {
                if (src.equals(file)) return !isIn;
            }
        }
        return isIn;
    }
